...
use twitch_api::{eventsub::EventSubscription, HelixClient};

use crate::error::{AppError, AppResult};
use crate::token_manager_v2::application::TokenManagerS;
...
impl TwitchApiClient {
    ...
    pub async fn create_eventsub_subscription<E: EventSubscription + Send>(
        ...
        let token = self.get_token().await.ok_or_else(|| {
            AppError::Internal("No token available".to_string())
        })?;
